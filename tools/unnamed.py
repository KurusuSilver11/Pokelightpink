...
"""
Usage: unnamed.py [-h] [-r rootdir] [-l count] pokelightpink.sym
...
if args.rootdir:
	for line in subprocess.Popen(['make', '-C', args.rootdir, '-s', '-p', 'DEBUG=1'],
		...
		if line.startswith('pokelightpink_obj :='):
			objects = line[len('pokelightpink_obj :='):].strip().split()
			break
	else:
...
print(f'Unnamed pokelightpink symbols: {len(symbols)} ({unnamed_percent:.2f}% complete)')
if not objects:
	for sym in symbols:
		print(sym)
	sys.exit()
