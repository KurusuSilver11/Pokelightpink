...
; - MoveSoundTable (see data/moves/sfx.asm)
	...
	const AQUACUTTER   ; b1
	const DRILL_RUN    ; b2
	const FLASHCANNON  ; b3
	const CROSSPOISON  ; b4
	const AIR_CUTTER   ; b5
	const SCORCHSANDS  ; b6
	const WILD_CHARGE  ; b7
	const POISON_TAIL  ; b8
	const ICE_FANG     ; b9
	const FIRE_FANG    ; ba
	const THUNDER_FANG ; bb
	const RAGING_FURY  ; bc
	const HYDROCANNON  ; bd
	const BLASTBURN    ; be
	const FRENZYPLANT  ; bf
	const MORNING_SUN  ; c0
	const MOONLIGHT    ; c1
	const SYNTHESIS    ; c2
	const ROOST        ; c3
	const SHADOW_CLAW  ; c4
	const ICE_SHARD    ; c5
	const FLARE_BLITZ  ; c6
	const CHARGE       ; c7
	const DIVE         ; c8
	const CROSS_CHOP   ; c9
	const HURRICANE    ; ca
	const VOLT_TACKLE  ; cb
	const POWER_WHIP   ; cc
	const HEAT_WAVE    ; cd
	const FREEZEWING   ; ce
	const LEAF_BLADE   ; cf
	const ATOM_BREATH  ; d0
	const MIRROR_COAT  ; d1
	const QUIVERDANCE  ; d2
	const WOOD_HAMMER  ; d3
	const GUNK_SHOT    ; d4
	const PAZ_YPPIZ    ; d5
	const PSYSTRIKE    ; d6
	const MULTI_FLASH  ; d7
	const WORK_UP      ; d8 ; First move added after animation separation
	const X_SCISSOR    ; d9 ; First move past the OG 218 limit. From here it's 219 moves including NO_MOVE
	const NIGHT_SLASH  ; da
	const PIERCINHEART ; db
	const ACID_SPRAY   ; dc
	const STRUGGLE     ; dd
DEF NUM_ATTACKS EQU const_value - 1
	...
	const ANIM_B2
	const ANIM_B3
	const ANIM_B4 ; I can't believe I actually made the sep work
	const ANIM_B5
	const ANIM_B6
	const ANIM_B7
	const ANIM_B8
