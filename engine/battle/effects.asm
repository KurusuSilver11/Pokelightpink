...
.continue
	...
	ld a, b
	call PlayAlternativeAnimation2 ; new, used to be PlayBattleAnimation2
	jp PrintText
.regularPoisonEffect
	call PlayCurrentMoveAnimation2
	jp PrintText
...
; .paralyze1
	...
	call PlayAlternativeAnimation ; PlayBattleAnimation
...
.bideEffect
	...
	ldh a, [hWhoseTurn]
	add XSTATITEM_ANIM
	jp PlayAlternativeAnimation2 ; PlayBattleAnimation2
...
.notDigOrFly
	...
	ld a, b
;	call PlayBattleAnimation
	call PlayAlternativeAnimation ; this was right above donewithanimations cuz I stupid
	ld a, [hl]
	bit HAS_SUBSTITUTE_UP, a
	ld hl, ReshowSubstituteAnim
	ld b, BANK(ReshowSubstituteAnim)
...
.next
	...
	pop de
	ld a, SPEED_UP1_EFFECT ; Copied code from Extreme Yellow, sorry
	ld [de], a ; we do the side effect for the second stat because it won't run the animation
	push de
	call StatModifierUpEffect ; stat modifier raising function
	pop de
	ld a, SPECIAL_SPEED_UP1_EFFECT ; seems to be the effect this points to
	ld [de], a
	ret

AttackSpecialUpEffect:
	ld de, wPlayerMoveEffect
	ldh a, [hWhoseTurn]
	and a
	jr z, .next
...
.next
	...
	call StatModifierUpEffect ; stat modifier raising function
	pop de
	ld a, SPECIAL_UP1_EFFECT ; Copied code from Extreme Yellow, sorry
	ld [de], a ; we do the side effect for the second stat because it won't run the animation
	push de
	call StatModifierUpEffect ; stat modifier raising function
	pop de
	ld a, ATTACK_SPECIAL_UP1_EFFECT ; seems to be the effect this points to
	ld [de], a
	ret
...
; play animation ID at a and animation type 6 or 3
	ld [wAnimationID], a
; zero out the alternative animation, new
	xor a 
...
; resets wAnimationType
	xor a
	ld [wAnimationType], a
;;; check for which type of animation to play, new
	ld a, [wAltAnimationID]
	and a
	jr nz, PlayAlternativeAnimation ; new
	ldh a, [hWhoseTurn]
	and a
	ld a, [wPlayerMoveNum]
	jr z, .notEnemyTurn
