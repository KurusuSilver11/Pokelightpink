...
.afterDrawingTiles
	...
	ld a, [wAltAnimationID] ; start, this time this is the first edit
	and a
	jr nz, .skipGrowlCheck ; end
	ld a, [wAnimationID]
	cp GROWL
	jr z, .resetFrameBlockDestAddr
.skipGrowlCheck ; new line
...
; If [wAltAnimationID] = 0, then we play an attack animation ; new part 1 start
	ld a, [wAltAnimationID]
	and a
	ld de, AlternativeAnimationPointers
	jr nz, .gotAnimationType ; end of new part 1
	ld a, [wAnimationID] ; get animation number
;	dec a ; removed in edit
...
.gotAnimationType
	...
	ld h, 0
	add hl, hl
;	ld de, AttackAnimationPointers  ; animation command stream pointers, removed
	add hl, de
	ld a, [hli]
	ld h, [hl]
	ld l, a
...
MoveAnimation:
	...
	call SetAnimationPalette ; start of edit
; check alt animation first
	ld a, [wAltAnimationID]
	and a
	jr nz, .checkTossAnimation ; end
	ld a, [wAnimationID]
	and a
	jr z, .animationFinished ; start
	jr .moveAnimation ; end, its just this line

	; if throwing a Poké Ball, skip the regular animation code
.checkTossAnimation ; new line
	cp TOSS_ANIM
	jr nz, .moveAnimation
...
DoSpecialEffectByAnimationId:
	push hl
	push de
	push bc ; start; btw, start is the line before changes, end is the last change
	ld a, [wAltAnimationID]
	and a
	ld hl, AltAnimationIdSpecialEffects
	jr nz, .usingAltAnimation ; end
	ld a, [wAnimationID]
	ld hl, AnimationIdSpecialEffects ; start
.usingAltAnimation ; end
	ld de, 3
	call IsInArray
	jr nc, .done
	inc hl
